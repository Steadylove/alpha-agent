...
网页 `/push` 的「期权大单金额门槛」保存到 `push-routes.json` 的 `optionFlowMinPremiumUsd`。旧配置缺字段时默认 500,000 美元；包含等于门槛的金额，0 表示不过滤。单笔和确认名单按每笔权利金筛选，金额未知时不通过正数门槛。名单只展示符合门槛的合约，原始记录仍完整入库，研究统计、GEX 和日结不受此门槛影响。

worker 每轮采集读取网页设置；单笔网站转发入口再校验一次金额，两者读取配置失败时均暂停本次转发。被过滤的旧消息不会因之后降低门槛而主动补推。单笔使用同一 PNG 发送至已配置的 Discord / Telegram，确认名单保留原有接收通道。

卡片消息时间统一为 `YYYY-MM-DD HH:mm 美东时间`（按 America/New_York 自动处理夏冬令时）。到期日统一为 `YYYY-MM-DD`；只有月日时按消息所属美东日期推断最近的未来年份，明确给出年份时不改写。只有月份 / 相对期限时保留其精度，例如 `2027-03（月）`、`约 2 周后`，不推定第三个周五。

发布需同时部署网站与新版 `option-flow` worker（现有 GitHub 部署流程会重新 bundle）；旧 worker 未附带金额的单笔会被新版接收入口的正数门槛拦截，避免绕过过滤。首次部署无需重置采集游标或重放消息。
