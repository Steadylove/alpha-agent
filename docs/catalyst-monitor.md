...
| --- | --- | --- |
| Alpaca News | Recent company news and announced corporate events | Existing `ALPACA_API_KEY` / `ALPACA_API_SECRET`; last 7 days, 15-minute delay, all current observed symbols, up to 40 pages / four minutes per run; unfinished pagination remains partial |
| BLS / official fallback | Economic release schedules | Public BLS ICS; if blocked, verified New York Fed entries may supplement core BLS indicators. Fallback coverage remains partial and dates are marked tentative |
| BEA | Official release schedule | Public calendar; officially undated releases are explicitly counted as pending dates, not fabricated |
| Federal Reserve | FOMC meeting dates | Official calendar; no invented announcement clock time |
| FMP / Nasdaq | Scheduled earnings for observed stocks, next 7 days | With `FMP_API_KEY`, use FMP (entitlement required); otherwise use the public Nasdaq calendar. Provider identity is explicit, all third-party schedules remain estimated, and unavailable sessions do not acquire an invented clock time |
| SEC | Recent company filing notices | Optional real `SEC_USER_AGENT` contact identity; query the current observation pool in holding/signal/Opportunity priority order, paced below 10 requests/second with a four-minute budget; unfinished or unmapped companies remain partial. Filing text is not inferred from form type |
...
## Daily Review and Tomorrow Map

The collector also saves a small independent supplement at `snapshots/catalyst/review/YYYY-MM-DD.json`, after a daily review exists for the same settled market date. It never edits the original review, AI Analyst Note input hashes, strategy output or notification cards. Both website sections read only this saved supplement with an independent failure boundary; a page request never fetches news or calls a model.

- **Catalyst Today:** at most three material items, with at most one upcoming calendar reminder. Published news must belong to the review's ET date. Priority is Portfolio → Signal → Sector → Market; Opportunity-only and low-importance news do not enter the summary. Same-object/event-type repeats are condensed by the selector. Items show a brief title and only verified same-date observations, not article bodies or another AI essay.
- **Tomorrow Events:** at most three highly relevant scheduled events in the next 72 hours **including the first 24 hours**, relative to the supplement's capture timestamp. Minute precision uses the exact future interval. Date/session-only schedules retain their uncertainty and are conservatively selected; After Close is never replaced with an invented clock time. Cancelled/published results and unknown timing do not become future reminders; estimated schedules remain explicitly labelled as estimates. The original Tomorrow Map five watch events and retrospective validation remain unchanged.
- **Metrics:** Price is a ready T0 close compared with the prior regular close, only when T0 is the review date. RPS uses the same daily composite scale across the previous review date and review date. A later mature T+ observation is not substituted. Actual Sector RPS comes from the matching archived review ETF's `d1`, not Catalyst's sector excess-over-SPY metric. After-close announcements cannot acquire tomorrow's returns in today's review.
- **Time and retention:** the supplement records `reviewBuiltAt` and its own later `capturedAt`, displayed explicitly as a post-review supplement. It is not claimed to have been known when the original review was published. The first informative result (or successfully observed complete empty result) is frozen and survives the full Catalyst archive's 45-day pruning. An empty failed/partial attempt may retry for that same current date. Explicit `--refresh-review-digest` runs may publish a supplement revision for the latest settled review only: validate the new result, archive the previous bytes under `catalyst/review/history/YYYY-MM-DD/`, then atomically replace the current supplement. The UI shows the revision and actual capture time; semantically unchanged or degraded coverage reuses the saved result. Supplement history is not subject to the full-report 45-day pruning. Historical dates without a supplement remain missing; latest news is never used as a silent historical replacement.
- **Coverage:** `No Material Catalyst` means none in the successfully collected source scope. Missing, failed and partial-source empty results use distinct wording. A saved upcoming list is an archived schedule as of its capture; later updates remain on `/catalyst`.

## Associations and research values

- Priority: 2H/4H **model** holdings → real captured signals from the last 10 calendar days → up to 250 Opportunity candidates/elite/high-RPS50 stocks → sectors → market. Maximum 250 observed symbols, preserving holding/signal priority and explicitly reporting truncation. These are the current observation pool, not all listed equities. Model holdings are not brokerage holdings.
...
npm run catalyst:build -- --analyze --refresh-review-digest
...
Install `deploy/market-http/cron/install-catalyst.sh` on the existing VPS after deploying the bundle. It installs two independent jobs: collection every hour at **:07 and :37** (every 30 minutes), and saved AI analysis plus an explicit review supplement at **10:05 Asia/Shanghai Tuesday–Saturday**, **12:55 Asia/Shanghai daily**, and **08:55 America/New_York Monday–Friday** (Beijing 20:55 during US daylight time, 21:55 during US standard time). These are start times; completion depends on upstream responses. The morning slot follows the 08:30 main job's maximum 90-minute run window; the midday slot follows the 12:30 macro refresh; the US morning slot leaves time after 08:30 ET releases and the news feed's 15-minute delay. Collection is offset from the existing daily and macro jobs at :30 so it cannot win their simultaneous non-blocking lock attempt; its 10-minute service timeout plus timer accuracy leaves time before those scheduled jobs. A shared daily-job lock prevents reads while the main data update is writing. Collection skips a busy lock; analysis waits up to 30 minutes, then reports failure. The analysis service permits one failure retry after five minutes (at most two starts in two hours). Collector failures wait for the next half-hour timer, avoiding retries across the main job's :30 slot. No original daily cron or message sender is invoked.
