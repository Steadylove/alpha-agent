...
const mocks = vi.hoisted(() => ({ push: vi.fn(), lookup: vi.fn(), png: Buffer.from('rendered-image') }));
...
vi.mock('@/lib/discord/signalCardOg', () => ({ renderSignalOgPng: async () => mocks.png }));
...
beforeEach(() => { vi.resetAllMocks(); vi.stubEnv('DISCORD_SIGNAL_WEBHOOK_URL', 'https://discord.example/hook'); });
