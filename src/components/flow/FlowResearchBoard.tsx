...
export function FlowResearchBoard({ report: r, dates, outcomes, error }: ResearchPage) {
  ...
  return <div className={s.root} aria-busy={pending}>
    <PageHeading eyebrow="FLOW OBSERVATORY" title="异常期权流" english="Flow research"
      description="资金出现在哪里，强度是否跟上，是否持续发生。" action={
      <div className={s.dateControl}>
        <ActionIcon variant="subtle" color="gray" size="sm" aria-label="上一交易日" disabled={pending || index <= 0} onClick={() => go(dates[index - 1])}><ChevronLeft size={17} /></ActionIcon>
        <Select size="xs" w={160} searchable aria-label="研究日期" value={r?.date ?? null}
          disabled={pending || !dates.length} onChange={value => { if (value) go(value); }}
          data={[...dates].reverse()} placeholder="选择研究日期" nothingFoundMessage="没有该日期的研究" />
        <ActionIcon variant="subtle" color="gray" size="sm" aria-label="下一交易日" disabled={pending || index < 0 || index >= dates.length - 1} onClick={() => go(dates[index + 1])}><ChevronRight size={17} /></ActionIcon>
      </div>
    } />
    ...
    {!r ? <div className={s.empty}>数据就绪后，这里会呈现真实记录、强度对照与跟踪结果。<Button variant="default" size="xs" mt="md" loading={pending} onClick={() => start(() => router.refresh())}>重新读取</Button></div> : <>
      ...
      <section className={s.section}>
        ...
        <Disclosure className={s.details} title={<>查看跟踪明细 <span>{outcomes.length} 个标的日样本</span></>}><div className={s.tableWrap}><table><thead><tr>{["报道日期", "标的", "当日强度", "T+1", "T+5", "T+10", "超额 SPY 5D", "MFE 10D", "MAE 10D", "状态"].map(h => <th key={h}>{h}</th>)}</tr></thead><tbody>{outcomes.map(o => <tr key={`${o.date}/${o.ticker}`}><td>{o.date}</td><td><strong>{o.ticker}</strong></td><td>{o.strength}</td>{[o.t1, o.t5, o.t10, o.excess5, o.mfe, o.mae].map((v, i) => <td key={i} className={tone(v)}>{signed(v, true)}</td>)}<td>{o.status}</td></tr>)}</tbody></table></div>{!outcomes.length && <p className={s.empty}>等待后续交易日行情，尚无可评价样本。</p>}</Disclosure>
        <p className={s.footnote}>分组均值仅作描述；重复标的并非独立样本，尚未完成同板块 / 相近 RPS 的匹配对照，不能据此认定预测有效。</p>
      </section>
      <Disclosure className={s.details} title={<>原始记录与数据口径 <span>{r.events.length} 条 · 可追溯来源</span></>}>
