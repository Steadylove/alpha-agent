...
export async function sealRelayIdentity(token: string, body = "", now = Date.now(), publicKey = TELEGRAM_RELAY_PUBLIC_KEY) {
  const key = await importSPKI(publicKey, "RSA-OAEP-256");
  const envelope = Buffer.from(JSON.stringify({ token, digest: digest(body), time: now }));
...
export async function verifyRelayIdentity(privateKey: string, identity: string, body: string, now = Date.now(), verifyToken = verifyVercelIdentity) {
  if (!privateKey || !identity || identity.length > 16000) return false;
  try {
    ...
    const value = JSON.parse(Buffer.from(plaintext).toString("utf8")) as { token?: unknown; digest?: unknown; time?: unknown };
    if (typeof value.token !== "string" || typeof value.time !== "number" || Math.abs(now - value.time) > 300_000 || value.digest !== digest(body)) return false;
