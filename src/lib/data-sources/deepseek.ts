...
1. 严格返回 JSON，字段固定为 marketNarrative(字符串)、themeChain(字符串数组)、beneficiarySectors(字符串数组)。
2. marketNarrative：一句中文，30~60 字，基于给定的 MSS/SKEW/VIX/信用/宽度 五个数字做定性判断（Risk-On/Neutral/Risk-Off + 结构性描述），禁止空话。
3. themeChain：从新闻里提炼今日主导产业链传导，形如 ["AI需求爆发", "云计算CAPEX上修", "GPU/HBM需求拉满"]，2~5 个环节。
4. beneficiarySectors：3~5 个受益板块中文短语，如 ["半导体", "存储", "数据中心", "电力设备"]。
5. 不要出现任何免责声明、投资建议、寒暄或解释。只返回 JSON，不加代码块。`;

const buildUserPrompt = (payload: DeepSeekPayload) => {
  const macro = payload.marketMetric;
  const macroLine = `MSS=${macro.mss}/100, SKEW=${macro.skewScore ?? "N/A"}/25, PCR/VIX=${
    macro.pcrScore ?? "N/A"
  }/25, 信用=${macro.creditScore ?? "N/A"}/25, 宽度=${macro.breadthScore ?? "N/A"}/25`;
  ...
  const topStocksLine = payload.topStocks
    .slice(0, 5)
    .map((s) => `${s.symbol}(${s.sector},score=${s.totalScore})`)
    .join("; ");
