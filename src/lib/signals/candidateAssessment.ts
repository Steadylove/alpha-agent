...
import { baselineEntryQualityOf, QUALITY_VERSION, qualityGrade, qualityPanel, type EntryQuality, type QualityDimension } from "./assessment";
...
export function candidateAssessmentOf(p: AlertPayload, rps?: number, context: CandidateContext = {}) {
  const baseline = baselineEntryQualityOf(p, rps), chart = buyChartOf(p.chart, p.barTime, p.price);
  ...
  const quality: EntryQuality = { version: QUALITY_VERSION, points, available, complete: available === 100, label: qualityGrade(points, available), dimensions };
...
export function candidatePreviewView(view: AlertView, candidate: CandidateAssessment): AlertView {
  return { ...view, quality: candidate.quality, assessment: qualityPanel(candidate.quality), candidate, qualityPreview: true,
