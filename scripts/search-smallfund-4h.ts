...
/**
 * Small Fund 4H 网格。整段五年既训练又评分，数字偏乐观。
 * 选平台不选尖峰。不改实验室默认。
 *
 *   npx tsx scripts/search-smallfund-4h.ts
 */
...
async function main() {
  ...
  console.log(
    `4H 池 ${universe.symbols.length} 只  轴 ${universe.axis[0]} → ${universe.axis.at(-1)}  ` +
      `载入 ${Date.now() - tLoad}ms`,
  );

  const tOne = Date.now();
  const current = evalCfg(universe, SMALL_FUND_4H_DEFAULT_CONFIG);
  console.log(`单组 ${Date.now() - tOne}ms  当前4H默认 ${label(current.config)}`);
  console.log(
    `  超额 ${pct(current.excess)}  年化 ${pct(current.cagr)} vs ${pct(current.bench)}  ` +
      `${current.trades} 笔  回撤 -${current.dd.toFixed(0)}%`,
  );

  const dailyOn4h = evalCfg(universe, {
    rpsMin: 40,
    stopMult: 4,
    trailMult: 5.5,
    takeProfitR: 1,
  ...
  const seed = coarseOk[0]?.config ?? current.config;
  ...
  for (const useBuy2 of [true, false]) {
    refine.push({
      requireVegas: false,
      requireRsi: false,
      rpsMin: seed.rpsMin,
  ...
  const pos = refineOk.filter((r) => r.excess > 0).length;
  console.log(`细扫正超额 ${pos}/${refineOk.length}`);

  const peak = refineOk[0] ?? current;
  const platform =
    refineOk.find(
      (r) =>
        r.config.rpsMin === 45 &&
        r.config.trailMult === 6 &&
        r.config.takeProfitR === 2 &&
        r.config.stopMult === 4 &&
        r.config.rpsWeightPower === 1,
    ) ?? current;

  printYears(`当前 4H 默认  ${label(current.config)}`, current);
  printYears(`日线平台搬到 4H  ${label(dailyOn4h.config)}`, dailyOn4h);
  printYears(`尖峰  ${label(peak.config)}`, peak);
  if (platform !== peak) printYears(`现 4H 平台组对照  ${label(platform.config)}`, platform);
  ...
  if (peak) {
    ...
    for (const axis of axes) {
      ...
      const bits = cells
        .map((r) => `${String(r.config[axis.key] ?? "空")}:${pct(r.excess)}`)
        .join("  ");
      console.log(`  ${axis.name.padEnd(6)} ${bits}`);
    }
  }
}
