...
const config: { token?: string; relaySecret?: string } = existsSync(configFile) ? (await import(pathToFileURL(configFile).href)).default : {};
...
const server = createTelegramRelayServer(store, secret, configured, () => ({ ok: !!bot && Date.now() - lastPollAt < 120_000, username: bot?.username }));
